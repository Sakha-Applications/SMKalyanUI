...
const handleDOBChange = (dob) => {
    ...
    // Convert time_of_birth if it is in an incorrect format
    ...
    return (
        <div>
        <Paper elevation={3} sx={{ maxWidth: "100%" , mx: "auto", p: 3, mt: 4, backgroundColor: "#f9f9f9" }}>
    ...
    <Box sx={{
        ...
        <TextField name="profileId" value={formData.profileId ?? ""} onChange={handleChange} fullWidth required sx={{ backgroundColor: "#fff", borderRadius: 1 }} />
...
<LocalizationProvider dateAdapter={AdapterDayjs} locale="en">
  <DatePicker
    ...
    value={formData.dob ? dayjs(formData.dob) : null} // Ensure null for empty values
    onChange={(newValue) => {
      const formattedDOB = newValue ? newValue.format("YYYY-MM-DD") : null; // ✅ Format Date
      handleDOBChange(formattedDOB); // ✅ Call function to update state
    }}
    slotProps={{
      textField: {
        fullWidth: true,
    ...
    <Box sx={{
        ...
        />
            ...
            <Box sx={{ display: "flex", justifyContent: "space-between", mt: 2 }}>
                <Button onClick={() => setTabIndex(tabIndex - 1)} disabled={tabIndex === 0} variant="contained" color="secondary">
                    Back
                </Button>
                {tabIndex < 4 ? (
                    <Button onClick={() => setTabIndex(tabIndex + 1)} variant="contained" color="primary">
                        Next
                    </Button>
                ) : (
