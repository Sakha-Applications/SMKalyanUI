...
const CareerEducationTab = ({ formData, handleChange, handleSubmit, isActive, tabIndex, setTabIndex }) => {
  ...
  const searchDesignations = async (searchText, profession) => {
    if (!searchText || searchText.length < 2) return;
    ...
    try {
      ...
      const professionResponse = await axios.get(`http://localhost:3001/api/profession?search=${encodeURIComponent(profession)}`);
      let professionId = null;
      
      if (Array.isArray(professionResponse.data) && professionResponse.data.length > 0) {
        const matchingProfession = professionResponse.data.find(p => p.ProfessionName === profession);
  ...
  const searchEducations = async (searchText) => {
    ...
    try {
      const response = await axios.get(`http://localhost:3001/api/education?search=${encodeURIComponent(searchText)}`);
  ...
  const fetchDesignationsForProfession = async (professionId) => {
    ...
    try {
      const response = await axios.get(`http://localhost:3001/api/designation?professionId=${professionId}`);
  ...
  return (
    ...
    >
      ...
      {/* Use FormNavigation with showSubmitButton set to false (keeping original behavior) */}
      <FormNavigation
        tabIndex={tabIndex}
