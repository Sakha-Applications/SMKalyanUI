...
export const handleSearchProfile = async (searchCriteria, setProfileData, setFetchError, setSearching, setUploadedPhotos, setDefaultPhoto, setUploadError, getUploadedPhotosFn, fetchDefaultPhotoFn) => {
    ...
    try {
        ...
        if (Array.isArray(response.data) && response.data.length > 0 && response.data[0].hasOwnProperty('profileId')) {
            ...
            await getUploadedPhotosFn(profileId, setUploadedPhotos, setFetchError); // <---- Use the extracted profileId
            await fetchDefaultPhotoFn(profileId, setDefaultPhoto, setFetchError); // <---- Use the extracted profileId
        } else {
...
export const handleUploadPhotos = async (profileData, photos, isDefaultPhoto, setUploading, setUploadError, setPhotosState, setPhotoPreviews, setIsDefaultPhoto, getUploadedPhotosFn, fetchDefaultPhotoFn, setUploadedPhotos, setDefaultPhoto) => {
    console.log('Debug (Utils): handleUploadPhotos - profileData received:', profileData);
    if (!profileData || !profileData.profileId) { // Access profileId directly
    ...
    formData.append('profile_id', profileData.profileId); // Access profileId directly
    formData.append('email', profileData.email || ''); // Access email directly
    ...
    try {
        ...
        await getUploadedPhotosFn(profileData.profileId, setUploadedPhotos, null); // Access profileId directly
        await fetchDefaultPhotoFn(profileData.profileId, setDefaultPhoto, null); // Access profileId directly
