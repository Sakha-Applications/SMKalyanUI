...
// Try to read logged-in user profile type from common localStorage shapes.
...
const getLoggedInUserProfileFor = () => {
  try {
    ...
    // Common property names seen in projects
    return (
      obj?.profileFor ||
      obj?.profile_for ||
      obj?.genderCategory || // if you stored as category
      obj?.gender ||         // if you stored "Bride"/"Bridegroom" as gender (not ideal but possible)
      ""
    );
  } catch (e) {
    return "";
    ...
    useEffect(() => {
  // If Looking For is empty, default it to opposite of logged-in user's profile
  ...
  // NOTE: do not add searchQuery to deps; we only want to set once on mount
...
// If user didn't select Looking For, force default = opposite of logged-in profile
if (!payload.profileFor) {
  const myProfileFor = getLoggedInUserProfileFor();
  payload.profileFor = getOppositeProfileFor(myProfileFor);
}

console.log("Sending search request with:", payload);
