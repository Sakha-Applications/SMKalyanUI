...
const communicationAddressDetails =
  parseCombinedAddressDetails(
    enriched.communication_address
  );

const residenceAddressDetails =
  parseCombinedAddressDetails(
    enriched.residence_address
  );
...
reference1PhoneCountryCode: reference1PhoneDetails.code,
    ...
    reference2PhoneNumber: reference2PhoneDetails.number,
      ...
      console.error("Ã¢ÂÅ’ Error fetching profile data:", error);
      setError("Failed to fetch profile data.");
    } finally {
      setLoading(false);
    }
  ...
  // Ã¢Å“â€¦ FIXED: Guru Matha search effect
  ...
  const delay = setTimeout(async () => {
    ...
    if (professionInput.length >= 2) {
      ...
      try {
        ...
        console.error("Ã¢ÂÅ’ Error fetching profession options:", error);
        setProfessionOptions([]);
      } finally {
  ...
  const delay = setTimeout(async () => {
    ...
    if (designationInput.length >= 2) {
      ...
      try {
        ...
        console.error("Ã¢ÂÅ’ Error fetching designation options:", error);
...
// Ã¢Å“â€¦ CORRECTED: Initialize input fields when entering/exiting edit mode
...
reference2PhoneCountryCode,      
    ...
    const finalPayload = {
      ...
      // Ã¢Å“â€¦ FIXED: Extract label from objects for backend
...
reference2_phone: (reference2PhoneCountryCode && reference2PhoneNumber) ? `${reference2PhoneCountryCode}${reference2PhoneNumber}` : (reference2PhoneNumber || null),
  ...
  <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
    <div>
      <h1 className={`text-2xl font-bold ${designClasses.textPrimary}`}>
        My Profile
      </h1>

      <p className={`mt-1 text-sm ${designClasses.textSecondary}`}>
        Review and maintain your personal, family and profile information.
      </p>
    </div>

    <div className="grid grid-cols-1 gap-4 text-sm sm:grid-cols-3">
      <div>
        <div className={designClasses.textSecondary}>Profile ID</div>
        <div className={`font-semibold ${designClasses.textPrimary}`}>
          {profileData.profile_id || profileData.profileId || "-"}
        </div>
      </div>

      <div>
        <div className={designClasses.textSecondary}>Name</div>
        <div className={`font-semibold ${designClasses.textPrimary}`}>
          {profileData.name || "-"}
        </div>
      </div>
      ...
      <div>
        <div className={designClasses.textSecondary}>Login ID</div>
        <div className={`font-semibold ${designClasses.textPrimary}`}>
          {profileData.userId || "-"}
        </div>
      </div>
        ...
        <div className="p-6">
          {isEditing ? (
            <EditModeProfile
              ...
              // Ã¢Å“â€¦ FIXED: Horoscope Dropdown Props
